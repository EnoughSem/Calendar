...
class TaskOfDayTaskActivity : AppCompatActivity() {

    private lateinit var bilding: ActivityTaskOfDayBinding

    companion object {
        const val TASK = "Task_id"
        const val DATE = "Date"
    }
    ...
    private fun createList(context : Context){
        CoroutineScope(Dispatchers.Default).launch {
            val equipment_id = intent.getStringExtra(TASK)
            val date = intent.getStringExtra(DATE)
            val url = "https://www.mamont-server.ru:8888/api/project_tasks/$equipment_id/$date/$date"
            HttpClient().use { client ->
                val dataString = client.get<String>(url)
                val typeToken = object : TypeToken<ArrayList<TasksJson>>() {}.type
                val studiosList =
                    Gson().fromJson<ArrayList<TasksJson>>(dataString, typeToken)
                CoroutineScope(Dispatchers.Main).launch {
                    bilding.listView.isClickable = false
                    bilding.listView.adapter = TasksJsonAdapter(context, studiosList)
                }
            }
        }
    }
